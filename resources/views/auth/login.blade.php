    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex items-center justify-end mt-4">
            @if (Route::has('register'))
                Not yet a member? <a href="{{ route('register') }}" class="rounded-md px-3 py-2 text-black ring-1 ring-transparent transition hover:text-black/70 focus:outline-none focus-visible:ring-[#FF2D20] dark:text-white dark:hover:text-white/80 dark:focus-visible:ring-white">
                Become a Member</a>
            @endif
                                </div>
    </form>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @include('auth.verify-email') <!-- Include the Verify Email Blade -->
            </div>
        </div>
    </div>
</div>
